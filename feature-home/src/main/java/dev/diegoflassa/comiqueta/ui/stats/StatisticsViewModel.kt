...
class StatisticsViewModel @Inject constructor(
    ...
    private fun loadStats() {
        TimberLogger.logD("CMD_StatisticsViewModel", "DEBUG: loadStats called")
        viewModelScope.launch {
            getCollectionStatsUseCase()
                .debounce(500L)
                .catch { e ->
                    FirebaseCrashlytics.getInstance().recordException(e)
                    TimberLogger.logE("CMD_StatisticsViewModel", "DEBUG: Error loading stats", e)
                    _uiState.update { StatisticsUIState.Error(e.message ?: "Unknown error") }
                }
                .collect { stats ->
                    TimberLogger.logD("CMD_StatisticsViewModel", "DEBUG: Stats received: $stats")
