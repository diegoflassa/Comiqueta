...
object CoverUtils {
    ...
    fun saveBitmapToCache(context: Context, bitmap: Bitmap, originalFileName: String): Uri? {
        ...
        if (!coversDir.exists() && !coversDir.mkdirs()) {
            TimberLogger.logE(
                TAG, "[Comiqueta][CoverUtils] Failed to create covers directory: ${coversDir.absolutePath}"
            )
            return null
        }
        ...
        try {
            ...
            TimberLogger.logD(TAG, "[Comiqueta][CoverUtils] Saved cover to: ${imageFile.absolutePath}")
            return imageFile.toUri()
        } catch (ioe: IOException) {
            TimberLogger.logE(
                TAG, "[Comiqueta][CoverUtils] Error saving bitmap to cache: ${ioe.message}", ioe
            )
        }
        return null
    }
