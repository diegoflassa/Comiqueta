...
Keep the AI **working set** lean — the docs an agent loads to generate code: `conductor/index.md` + `conductor/rules/*` + `conductor/ki/KI_INDEX.md`. KIs are lazy-loaded, so individual `KI-NNN_*.md` files do not count against the per-task budget.
...
- `KI_INDEX.md`: **one line per KI**, no prose.

## Index pruning rules

- Every KI create / rename / delete updates `ki/KI_INDEX.md` in the **same turn** — no stale rows, no dangling links.
...
1. Measure bytes/tokens of `index.md`, `rules/*`, and `ki/KI_INDEX.md`.
