...
# /rules-status — Rule Set Audit

> Invoked by `/rules-status`. Read-only. Produces a findings report; **fixes require approval**.
...
6. **Pointer freshness.** For each `.agents/rules/*.md`: the section it cites exists, and its `description`
