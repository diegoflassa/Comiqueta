...
> Invoked as `/remove-filter <FILTER_NAME>`. Removes `TimberLogger.logD` / `logV` calls tagged with a specific filter **while protecting application logic**. Also the runbook for adding a filter.
