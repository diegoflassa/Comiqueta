...
# /token-audit — Context Budget Audit

> Invoked by `/token-audit`. Read-only.

---

## Scope

`AGENTS.md`, everything under `conductor/rules/` and `conductor/knowledge/`, and the `.agents/` tree.
...
4. **Flag any `.agents/rules/*.md` over ~40 lines.** A pointer that long has stopped being a pointer and has
